...
export const BioBuilder: React.FC<BioBuilderProps> = ({ userId, onNavigateToDashboard }) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#080B11]">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-white/50">Đang tải cấu hình Bio...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#080B11] text-white flex flex-col">
      {/* Sub Header */}
      <header className="border-b border-white/5 bg-brand-dark/80 backdrop-blur-md sticky top-0 z-20 px-6 py-4 flex justify-between items-center">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="flex-1 p-6 space-y-8 overflow-y-auto max-h-[calc(100vh-70px)] border-r border-white/5">
          
          {/* Section 1: Thông tin chung */}
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-white/80 uppercase tracking-wider flex items-center gap-2 border-b border-white/5 pb-2">
            ...
            <div className="grid grid-cols-1 gap-4">
              {/* Slug link */}
              <div>
                <label className="block text-xs text-white/50 font-semibold mb-2">Đường dẫn trang Bio (Slug)</label>
                <div className="flex rounded-xl overflow-hidden border border-white/10 glass-input focus-within:border-brand-orange/40">
                  <span className="bg-white/5 px-3 py-3 text-xs text-white/40 border-r border-white/5 flex items-center select-none">
                    quickbio.vercel.app/
                  </span>
                  <input 
                    type="text" 
                    ...
                    className="flex-1 bg-transparent px-3 py-3 text-xs text-white outline-none"
                  />
                  {slugStatus === 'checking' && (
                    <span className="p-3 flex items-center"><Loader2 className="w-3.5 h-3.5 text-white/40 animate-spin" /></span>
                  )}
                ...
                <span className="text-[10px] text-white/40 mt-1 block">Chỉ dùng chữ, số, gạch ngang (-) và gạch dưới (_).</span>
              </div>

              {/* Title & Bio */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-white/50 font-semibold mb-2">Tiêu đề hiển thị (Họ tên)</label>
                  <input 
                    type="text" 
                    value={bio.title}
              ...
              <div>
                <label className="block text-xs text-white/50 font-semibold mb-2">Giới thiệu bản thân (Bio text)</label>
                <textarea 
                  value={bio.bio_text}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-white/50 font-semibold mb-2.5">Hình nền (Background)</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-white/50 font-semibold mb-2.5">Màu điểm nhấn (Accent Color)</label>
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-brand-card/40 border border-white/5 mt-5 sm:mt-0">
                  <div>
                    <span className="text-xs font-semibold block text-white">Hiệu ứng Kính (Glassmorphism)</span>
                    <span className="text-[10px] text-white/40 block">Tạo chiều sâu mờ ảo cho thẻ</span>
                  </div>
                  <button
                    onClick={() => updateTheme({ glassmorphism: !bio.theme.glassmorphism })}
                    className={`w-11 h-6 rounded-full transition-colors relative flex items-center ${
                      bio.theme.glassmorphism ? 'bg-brand-orange' : 'bg-white/10'
                    }`}
                  >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={social.key}>
                  <label className="flex items-center gap-1.5 text-xs text-white/60 font-semibold mb-2">
          ...
          <div className="p-4 rounded-xl bg-brand-orange/5 border border-brand-orange/15 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-xs font-bold text-white block">Trạng thái Trang Bio</span>
              <p className="text-[10px] text-white/40">Đặt thành Công khai để hiển thị cho mọi người thấy và mua sản phẩm.</p>
            </div>
            <select
              value={bio.status}
              onChange={(e) => updateBioFields({ status: e.target.value as 'draft' | 'published' })}
              className="px-3 py-2 text-xs rounded-xl bg-[#080B11] border border-white/10 text-white outline-none focus:border-brand-orange"
            >
        ...
        <div className="hidden lg:flex flex-col items-center justify-center p-6 bg-brand-dark/30 max-h-[calc(100vh-70px)] select-none">
          <div className="text-xs font-semibold text-white/40 mb-3 flex items-center gap-1">
            <Smartphone className="w-4 h-4 text-brand-orange animate-bounce" />
            XEM TRƯỚC TRÊN MOBILE (LIVE PREVIEW)
          </div>

          {/* Mobile frame container */}
          <div className="relative w-[340px] h-[640px] rounded-[48px] border-[10px] border-slate-800 bg-[#080B11] shadow-2xl overflow-hidden flex flex-col">
            ...
            >
              ...
              <div className="flex gap-2 justify-center flex-wrap">
                {Object.entries(bio.social_links).map(([key, url]) => {
                  ...
                  return (
                    <div 
                      key={key}
                      className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-[10px]"
                    >
              ...
              <div className="w-full space-y-3 pt-2">
                ...
                >
                  <div className="w-16 h-16 bg-white/5 rounded-lg border border-white/5 flex-shrink-0 flex items-center justify-center text-[10px] text-white/30">
                    Product
                  </div>
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      <button 
                        className="px-2.5 py-1 text-[9px] font-bold rounded-lg text-white"
