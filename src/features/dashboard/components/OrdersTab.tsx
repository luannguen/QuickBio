    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-4 text-xs">
          <div className="bg-brand-card/50 px-3 py-1.5 rounded-lg border border-border">
            <span className="text-muted-foreground">Thành công:</span> <strong className="text-semantic-success ml-1">{totalPaidOrders}</strong>
          </div>
          <div className="bg-brand-card/50 px-3 py-1.5 rounded-lg border border-border">
            <span className="text-muted-foreground">Doanh thu:</span> <strong className="text-brand-orange ml-1">{totalRevenue.toLocaleString('vi-VN')}đ</strong>
          </div>
        </div>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="glass-panel p-10 rounded-2xl border border-border text-center space-y-4">
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-foreground text-sm">Chưa có đơn hàng nào</h4>
            <p className="text-xs text-muted-foreground">Khi có người click mua sản phẩm và quét mã QR chuyển khoản, đơn hàng sẽ hiển thị tại đây.</p>
          </div>
        </div>
      ) : (
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-background border border-border rounded-xl text-xs text-foreground placeholder-white/40 outline-none focus:border-brand-orange/50 transition-colors"
              />
            </div>
            <div className="relative w-full sm:w-48">
              ...
              <select
                ...
                className="w-full pl-9 pr-4 py-2 bg-background border border-border rounded-xl text-xs text-foreground outline-none focus:border-brand-orange/50 appearance-none transition-colors"
              >
          ...
          <Card className="p-0 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-border bg-white/[0.01] text-muted-foreground font-semibold uppercase tracking-wider">
                    ...
                    filteredOrders.map(o => (
                      <tr key={o.id} className="hover:bg-white/[0.02] transition-colors">
                        <td className="p-4">
                          ...
                          <div className="text-[10px] text-muted-foreground mt-0.5">{o.customer_email}</div>
                        </td>
                        <td className="p-4 truncate max-w-[150px] font-medium text-foreground">{o.product?.name || 'Sản phẩm đã bị xóa'}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4 font-bold text-foreground text-right">{o.amount.toLocaleString('vi-VN')}đ</td>
            ...
            <div className="bg-brand-orange/5 p-4 rounded-xl border border-brand-orange/10 space-y-3 max-w-md mt-6">
              ...
              <div className="space-y-2">
                <select
                  ...
                  className="w-full px-2 py-2 text-[10px] rounded-lg bg-background border border-border text-foreground outline-none"
                >
                ...
                <button
                  ...
                  className="w-full py-2 bg-brand-orange hover:bg-brand-coral disabled:bg-muted/50 text-white disabled:text-muted-foreground text-[10px] font-bold rounded-lg transition-all touch-target min-h-[36px]"
                >
