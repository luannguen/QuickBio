...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (reduce) {
        ...
        c.style.setProperty('--sw-pr', pr.toFixed(3));
        c.style.setProperty('--sw-cop', cop.toFixed(3));
        c.style.setProperty('--sw-pe', cop > 0.5 ? 'auto' : 'none');
      }
    }
  ...
  const handleScroll = () => { if (!ticking) { ticking = true; requestAnimationFrame(read); } };
...
function injectCSS() {
  ...
  .sw-nav{display:flex;gap:8px;padding:6px;background:rgba(9,11,15,0.75);backdrop-filter:blur(10px);border:1px solid rgba(250,246,238,0.1);border-radius:999px;}
  .sw-nav__item{font:inherit;font-weight:500;font-size:.8rem;color:rgba(250,246,238,0.6);border:0;background:transparent;cursor:pointer;padding:6px 14px;border-radius:999px;transition:color .25s,background .25s,transform .2s;}
  .sw-nav__item:hover{color:#fff;background:rgba(250,246,238,0.05);transform:scale(1.03);} .sw-nav__item.is-active{color:#090B0F;background:var(--sw-accent);font-weight:700;}
  ...
  .sw-copy__num{
    ...
    transform:translateY(calc((0.5 - var(--sw-pr,0.5)) * -20px));
    opacity:var(--sw-cop,0);
  ...
  .sw-copy__eyebrow{
    ...
    color:var(--sw-accent);
    transform:scale(calc(1.3 - abs(0.5 - var(--sw-pr,0.5)) * 0.6)) translateY(calc((0.5 - var(--sw-pr,0.5)) * -10px));
    opacity:var(--sw-cop,0);
    will-change:transform,opacity;
  }
  .sw-copy__title{
    ...
    transform:translateX(calc((0.5 - var(--sw-pr,0.5)) * -80px));
    opacity:var(--sw-cop,0);
    will-change:transform,opacity;
  }
  .sw-copy__body{
  ...
  .sw-copy__tags{
    ...
    margin:24px 0 0;
    padding:0;
    transform:translateY(calc((0.5 - var(--sw-pr,0.5)) * 90px));
    opacity:var(--sw-cop,0);
    will-change:transform,opacity;
  }
  .sw-copy__tags li{
  ...
  .sw-copy__cta{
    ...
    margin-top:28px;
    pointer-events:auto;
    transform:translateY(calc((0.5 - var(--sw-pr,0.5)) * 120px)) scale(calc(1.1 - abs(0.5 - var(--sw-pr,0.5)) * 0.2));
    opacity:var(--sw-cop,0);
    will-change:transform,opacity;
  }
